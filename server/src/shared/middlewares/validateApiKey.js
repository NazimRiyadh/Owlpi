...
import Container from "#src/services/client/dependencies/dependencies.js";
...
const validateApiKey = async (req, res, next) => {
    try {
        ...
        if (!apiKey) {
            ...
            return res
                .status(401)
                .json(ResponseFormat.error("API key is required", 401));
        }

        const result =
            await Container.initialized.services.clientService.getClientByApiKey(
                apiKey,
            );
        ...
        if (!result) {
            ...
            return res
                .status(403)
                .json(ResponseFormat.error("Invalid API key", 403));
        }
        ...
        if (!client.isActive) {
            ...
            return res
                .status(403)
                .json(ResponseFormat.error("Client account is inactive", 403));
        }
        ...
        return res
            .status(500)
            .json(ResponseFormat.error("Internal server error", 500));
    }
};

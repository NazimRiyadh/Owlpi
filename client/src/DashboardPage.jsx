...
const MOCK_DASHBOARD_DATA = {
  ...
  topEndpoints: [
    { endpoint: '/api/v1/users', totalHits: 45200, avgLatency: 12, serviceName: 'User_Service' },
    { endpoint: '/api/v1/auth/login', totalHits: 12400, avgLatency: 85, serviceName: 'Auth_Service' },
    { endpoint: '/api/v1/analytics', totalHits: 38200, avgLatency: 120, serviceName: 'Telemetry_Node' },
    { endpoint: '/api/v1/settings', totalHits: 8200, avgLatency: 45, serviceName: 'Config_Server' }
  ],
...
export default function DashboardPage({ user, onRequireAuth, onBackHome }) {
  ...
  const loadDashboard = useCallback(async () => {
    ...
    try {
      ...
      const qs = new URLSearchParams({ startTime: String(start), endTime: String(end) });
      ...
      if (isGuest || profile?.isDemo || !data) {
        ...
        if (!isGuest && !profile?.isDemo) {
          setError('Infrastructure connection limited. Displaying cached telemetry.');
        }
      } else {
  ...
  }, [loadDashboard, profileState, profile]);
  ...
  if (profileState === 'loading') {
    return (
      <div className="flex h-screen w-full flex-col items-center justify-center bg-[#fffefb]">
        ...
        />
        <p className="text-[10px] font-bold text-[#939084] uppercase tracking-[0.4em] animate-pulse">Initializing Protocol...</p>
      </div>
    );
  }
    ...
    return (
      <div className="flex h-screen w-full items-center justify-center bg-[#fffefb]">
        <div className="text-center">
           ...
           <p className="text-[#ff4f00] font-bold uppercase tracking-[0.2em] text-[10px]">Access Revoked. Re-authenticating...</p>
        </div>
      </div>
    );
  }
        ...
        <main className="flex-1 overflow-y-auto scrollbar-hide bg-[#fcfcfc]/50">
          {error && (
            <div className="bg-[#ff4f00]/5 border-b border-[#ff4f00]/10 px-8 py-2 flex items-center justify-between">
              <span className="text-[9px] font-bold text-[#ff4f00] uppercase tracking-widest">{error}</span>
              <button onClick={() => setError('')} className="text-[#ff4f00] opacity-50 hover:opacity-100"><X size={12} /></button>
            </div>
          )}
          <div className="min-h-full">
            {currentView === 'settings' && <OrganizationPanel profile={profile} />}
            {currentView === 'team' && <TeamPanel profile={profile} />}
            {currentView === 'metrics' && <MetricsPanel data={data} loading={loading} />}
            {currentView === 'system' && isSuperAdmin && <SystemAdminPanel profile={profile} />}
          </div>
        </main>
      </div>
    </div>

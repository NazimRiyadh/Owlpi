...
export default function Sidebar({ profile, currentView, setCurrentView, onLogout, onBackHome }) {
  ...
  const sections = [
    {
      ...
      items: [
        ...
        { id: 'team', label: 'Team Members', icon: Users, hidden: !canManageUsers },
        { id: 'system', label: 'System Settings', icon: Shield, hidden: !isSuperAdmin },
      ]
    }
  ];
  ...
  const userRoleName = profile?.role?.replace('_', ' ').toUpperCase() || 'AUTHORIZED PERSONNEL';

  return (
    <aside className="w-64 border-r border-[#c5c0b1] bg-[#fffefb] flex flex-col h-full py-6 px-4 relative">
      {/* Brand Section */}
      <div className="flex items-center justify-between mb-8 px-2">
        <div className="flex items-center gap-3.5 group cursor-pointer" onClick={onBackHome}>
          <div>
                ...
                >
                  <div className="flex items-center gap-3">
                    <item.icon size={16} className={cn(
                      "transition-colors",
                      currentView === item.id ? "text-[#ff4f00]" : "text-[#939084] group-hover:text-[#201515]"
                    )} />
      ...
      {showProfileCard && (
        <div className="absolute bottom-32 left-4 right-4 bg-[#fffefb] border border-[#ff4f00] rounded-[4px] p-4 shadow-[0_8px_30px_rgb(0,0,0,0.12)] z-50 animate-in fade-in slide-in-from-bottom-2 duration-200">
           <div className="flex items-center justify-between mb-4">
              <span className="text-[9px] font-bold uppercase tracking-[0.2em] text-[#ff4f00]">Identity Verified</span>
              <button onClick={() => setShowProfileCard(false)} className="text-[#939084] hover:text-[#201515]">
           ...
           <div className="space-y-3">
              <div className="flex items-center gap-3 pb-3 border-b border-[#eceae3]">
                 ...
                 <div>
                    <p className="text-[12px] font-bold text-[#201515] leading-tight">{userDisplayName}</p>
                    <p className="text-[9px] text-[#939084] font-bold uppercase tracking-widest">{userRoleName}</p>
                 </div>
              </div>
              
              <div className="space-y-2">
                 <div className="flex items-center justify-between text-[10px]">
                    <span className="text-[#939084] font-bold uppercase">Org Context</span>
                    <span className={cn(
                      "font-bold uppercase tracking-tight",
                      isSuperAdmin ? "text-[#ff4f00]" : "text-[#201515] font-mono"
                    )}>
                      {isSuperAdmin ? 'Global Infrastructure' : (profile?.clientId?.substring(0, 8) || 'Unknown')}
                    </span>
                 </div>
                 <div className="flex items-center justify-between text-[10px]">
                    <span className="text-[#939084] font-bold uppercase">System Link</span>
              ...
              <div className="pt-2">
                 <div className="flex flex-wrap gap-1">
                    {isSuperAdmin && <Badge className="bg-[#ff4f00]/5 text-[#ff4f00] border-[#ff4f00]/20 rounded-[2px] text-[7px] uppercase font-bold">Root Access</Badge>}
                    <Badge className="bg-[#eceae3] text-[#201515] border-none rounded-[2px] text-[7px] uppercase font-bold px-1.5">Secure</Badge>
                 </div>
              </div>
           </div>
        </div>
      ...
      <div className="mt-auto pt-6 border-t border-[#c5c0b1]">
        <button 
          ...
          className={cn(
            ...
            showProfileCard ? "border-[#ff4f00] shadow-[0_4px_12px_rgba(255,79,0,0.1)]" : "border-[#c5c0b1] hover:border-[#ff4f00]"
          )}
        >
          ...
          <div className="flex-1 overflow-hidden">
            <p className="text-[11px] font-bold text-[#201515] truncate">{userDisplayName}</p>
            <p className="text-[9px] text-[#939084] font-bold uppercase tracking-widest truncate">{userRoleName}</p>
          </div>
          <ChevronRight size={14} className={cn("text-[#939084] transition-transform", showProfileCard && "rotate-90 text-[#ff4f00]")} />
        </button>
...
function Badge({ children, className }) {
  return (
    <span className={cn("px-1.5 py-0.5 rounded-[2px] border text-[7px] font-bold uppercase", className)}>
      {children}
    </span>
  );
}
